...
class Passbook : AppCompatActivity() {
    ...
    private var passbookDocUri: Uri? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val backButton: ImageView = findViewById(R.id.passbookdetailsBackBtn)
        val submitButton: Button = findViewById(R.id.submit_passbook_button)
        ...
        submitButton.setOnClickListener {
            if (passbookDocUri == null) {
                ...
                if (deliveryPartnerId != null) {
                    uploadPassbookDetails(deliveryPartnerId)
                } else {
    ...
    private fun openCamera() {
        try {
            ...
            passbookDocUri = FileProvider.getUriForFile(this, "${packageName}.provider", tempFile)

            val intent = Intent(MediaStore.ACTION_IMAGE_CAPTURE).apply {
                putExtra(MediaStore.EXTRA_OUTPUT, passbookDocUri)
            }
            startActivityForResult(intent, PICK_CAMERA_REQUEST_CODE)
        } catch (e: Exception) {
            Log.e("PassbookDetails", "Error opening camera", e)
        ...
        if (resultCode == RESULT_OK) {
            when (requestCode) {
                PICK_IMAGE_REQUEST_CODE -> {
                    passbookDocUri = data?.data
                    imageView.setImageURI(passbookDocUri)
                }
    ...
    private fun uploadPassbookDetails(deliveryPartnerId: String) {
        try {
            passbookDocUri?.let {
                val passbookDocFile = uriToFile(it)
                val passbookDocRequestBody = RequestBody.create("image/*".toMediaTypeOrNull(), passbookDocFile)
                val passbookDocPart = MultipartBody.Part.createFormData("img", passbookDocFile.name, passbookDocRequestBody)

                val apiService = RetrofitClient.apiService

                apiService.uploadPassbookDocument(deliveryPartnerId, passbookDocPart).enqueue(object : Callback<ResponseBody> {
                    override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                        loaderContainer.visibility = android.view.View.GONE // Hide progress bar
                        if (response.isSuccessful) {
                            Toast.makeText(this@Passbook, "Upload successful", Toast.LENGTH_SHORT).show()
                            val resultIntent = Intent().apply {
                                putExtra("isPassbookSubmitted", true)
                            }
                            setResult(Activity.RESULT_OK, resultIntent)
                            finish()
                        } else {
                            Log.e("Passbook", "Upload failed: ${response.code()}, ${response.message()}")
                            Toast.makeText(this@Passbook, "Upload failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                        loaderContainer.visibility = android.view.View.GONE // Hide progress bar
                        Log.e("Passbook", "Error uploading passbook document", t)
                        Toast.makeText(this@Passbook, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                })
            }
        } catch (e: Exception) {
            loaderContainer.visibility = android.view.View.GONE // Hide progress bar
            Log.e("Passbook", "Error while converting URI to file", e)
